...
预计后续 animate custom 窄 register 数量：`0-1` 个。

- `register-component` 是唯一保留的 stats-first 候选，范围是 `poptipAppear` / `poptipDisappear` / `labelItemAppear` / `labelItemDisappear`。当前直接源码约 9,057 raw / 1,710 gzip，假想 closure 约 13 files / 22,238 gzip；只有当 VChart / VTable 有明确 component-only optional profile 时才值得新增。
