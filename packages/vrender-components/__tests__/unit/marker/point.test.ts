...
import { Tag, Segment, MarkPoint } from '../../../src';
...
describe('Marker', () => {
  ...
  it('MarkPoint Text', () => {
    ...
    // tag构造的label
    expect((markPointContainer.children[1] as unknown as Tag).attribute.x).toBeCloseTo(209.5782628522115);
    expect((markPointContainer.children[1] as unknown as Tag).attribute.y).toBeCloseTo(282.87347885566345);
    expect(
      ((markPointContainer.children[1] as unknown as Tag).getChildByName('tag-content') as any).children[0].attribute
        .text
