...
然后通过 `setStates` 方法来切换图元状态；当确实要移除当前激活状态时，可以使用 `clearStates`。不要为了刷新同一组状态先 `clearStates()` 再设置同样的状态，应直接使用 `setStates(states, { animate, animateSameStatePatchChange })`。
接下来通过一段代码示例演示
...
// Node：通过 app envParams 传入 node-canvas 兼容包。
const nodeApp = createNodeVRenderApp({
  envParams: CanvasPkg
});

// 小程序、Lynx 等宿主：app 级参数只放环境能力，例如像素比或可复用的 canvasFactory。
