...
export const page = () => {
  ...
  group
    .animate()
    .play(
      new ClipGraphicAnimate({ endAngle: 0 }, { endAngle: Math.PI * 2 }, 2000, 'quadIn', { group, clipGraphic: arc })
    );
  group
    .animate()
    .play(
  ...
  // group.animate().to({ x: 300, y: 300 }, 2000, 'quadIn');

  // group.animate().play(new RadiusClip({ radius: 0 }, { radius: 300 }, 1000, 'quadIn', { circle: circle1 }));
  // group.animate().play(new RadiusClip({ radius: 0 }, { radius: 300 }, 1000, 'quadIn', { circle: circle2 }));
  ...
  //   console.log('abc', arc.attribute);
  //   stage.ticker.tickAt!(parseFloat(input.value));
  // });
