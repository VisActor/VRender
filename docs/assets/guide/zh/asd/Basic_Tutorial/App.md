...
小程序类环境的宿主 canvas 能力通常和页面组件生命周期绑定。建议按页面或组件实例复用 `App`，每个 canvas 视图创建自己的 `Stage`。如果宿主需要 canvas pool，这类 pool 是环境能力，必须保证对同一共享 `App` 下的所有使用者有效。
