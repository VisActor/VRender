...
| glyph | `graphic/glyph.ts` | `glyph-render.ts`、`glyph-module.ts` | canvas/math `glyph-picker.ts` | child graphic composition、glyphStateProxy | core/kits `register-glyph.ts` | lite 默认包含 | 是，组合图元能力 | 基础但有 state 特例 | 不建议删除；验证 glyph state 行为 |
