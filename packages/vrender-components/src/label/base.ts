...
export abstract class LabelBase<T extends BaseLabelAttrs> extends AbstractComponent<T> {
  ...
  protected _smartInvert(labels: ILabelGraphicAttribute[]) {
    ...
    for (let i = 0; i < labels.length; i++) {
       ...
       */
      this._relationMap.get(label._relatedIndex).attribute.fill = this._relationMap.get(label._relatedIndex).attribute
        .fillColor
        ? this._relationMap.get(label._relatedIndex).attribute.fillColor
        : this._relationMap.get(label._relatedIndex).attribute.fill;
