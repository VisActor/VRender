...
| 确认 VRender 变更都已进入目标提交/分支 | VRender-side | release-day | `9b6508...` 及后续必要 typing/doc 变更已进入待发布分支 |
...
external-stage-first 已经证明路径可用，但源码侧仍应把 ownership contract 固定下来，避免后续调用方重新把外部 stage 当成内部 owned stage 释放。
