...
function normalizeWorkspaceDeps(deps, fallbackVersion) {
  ...
  for (const [name, version] of Object.entries(deps)) {
    if (typeof version === 'string' && version.startsWith('workspace:')) {
      ...
      normalized[name] = workspaceVersion === '*' ? fallbackVersion : workspaceVersion;
    } else {
...
function writeSanitizedPackageJson(sourcePackageDir, targetPackageDir) {
  ...
  fs.writeFileSync(path.join(targetPackageDir, 'package.json'), `${JSON.stringify(sanitized, null, 2)}\n`);
}
...
function syncPackage(packageFolder, packageName) {
  ...
  if (!fs.existsSync(packageJsonPath)) {
    throw new Error(`Missing package.json for ${packageName}: ${packageJsonPath}`);
  }

  const builtDirs = ['cjs', 'es', 'dist'].filter(dir => fs.existsSync(path.join(sourcePackageDir, dir)));
...
assertDir(path.join(projectRoot, 'node_modules'), 'Run "npm install" in multi-platform/wx-vrender before sync:local.');
