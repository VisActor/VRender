...
> Stable closeout note (2026-05-25): 本文主体最初形成于 browser-alpha 语境。当前已被 stable closeout 更新的结论包括：root app creator public typing 已补齐，node 已进入 stable Tier 1，text `stateProxy` 已覆盖，multi-env matrix 已收口为 Tier 1 `browser` / `node` / `wx` / `lynx` / `harmony`、Tier 2 `taro` / `feishu` / `tt`、Tier 3 `native`。以下内容保留评审脉络，但执行优先级以本 note、`D3_STABLE_RELEASE_CLOSEOUT_PLAN.md` 与 `D3_UPPER_LAYER_ADOPTION_GUIDE.md` 为准。
...
stable release closeout 后，当前仍需要继续处理的高优先级摩擦主要是：

1. `VChart` source-level app-provider-first 对齐
2. external stage ownership contract hardening
3. taro / feishu / tt 真实端 smoke 后的 Tier 升级判断
4. advanced on-demand public surface 是否继续治理
5. 是否需要更强 runtime isolation

补充状态：

- adoption guide 已补充 `VChart` 场景下的 internal-owned / external-owned ownership 矩阵
- node app-scoped runtime 已进入 stable Tier 1
- text `stateProxy` 已由 VTable-lite workload 覆盖
- multi-env support matrix 已完成 stable 分层：Tier 1 为 `browser` / `node` / `wx` / `lynx` / `harmony`
...
- 对 **stable Tier 1 + 根包 `@visactor/vrender` + 由调用方自己创建并持有 stage** 的主路径而言，当前新接入方式 **总体可接受**，已经足以作为上层继续验证和迁移的新默认入口。
- 当前最大的摩擦点 **不是** “多了一个 `app` 概念” 本身，而是：
  - `app` / `stage` 的 **ownership 与 release 语义表达不够清楚**
  - **多环境支持** 已在 stable matrix 中分层，但 Tier 2 环境仍需要真实端 smoke 后再升级承诺
  - **按需装配** 仍然存在，但没有跟着新主路径一起升级成同等清晰的 app-scoped contract
  - **external stage ownership** 仍有真实边界项
...
2. **运行时选择**：stable Tier 1 环境使用各自的 root app creator，例如 `createBrowserVRenderApp()`、`createNodeVRenderApp()`、`createWxVRenderApp()`、`createLynxVRenderApp()`、`createHarmonyVRenderApp()`；这不再是一个 root `createStage()` 同时兜底所有环境，见 `D3_UPPER_LAYER_ADOPTION_GUIDE.md`。
...
1. **多环境能力没有被删除，但已经分层**
   ...
   - app-scoped env installer surface 已覆盖 browser / node / taro / feishu / tt / wx / lynx / harmony
2. **根包推荐入口已经扩展为 stable support matrix，不是“所有历史环境一律一等”**
   - Tier 1: `browser` / `node` / `wx` / `lynx` / `harmony`
   - Tier 2: `taro` / `feishu` / `tt`
   - Tier 3: `native`
   - root creator 会直接跑对应 env + 默认 graphics + pickers 安装链
...
- 对普通 stable Tier 1 上层，当前新路径是“**全量默认装配的标准入口**”
- 对 Tier 2 环境或强依赖按需装配的上层，当前新路径还不能等价替代所有旧能力
...
- stable closeout 后，当前真正会影响上层接入成功率的摩擦点主要剩 5 类：**ownership/release 语义、external stage 边界、Tier 2 端真实 smoke、按需装配分层缺口、runtime isolation 边界**。
- 其余诸如“browser/node 双入口”“root vs core/kits 分层”“兼容 `createStage()` 仍保留”属于真实摩擦，但多数仍可归为可控迁移成本。

### 3.1 Resolved: 新入口 TypeScript API 表达已补齐

判断：`已完成治理`

关键事实：

- stable Tier 1 root app creator public typing 已返回 `IApp`。
- `packages/vrender/__tests__/types/entries-typing.ts` 已覆盖 browser / node 入口类型约束。
- build artifact consistency 已覆盖 public entry export。

当前结论：

- 该项不再是 D3 stable release blocker。
- 后续只需在新增 root app creator / public entry 时同步类型测试与 build artifact 检查。
...
### 3.4 Resolved: node readiness 已进入 stable Tier 1

判断：`已完成治理，保留 ABI 约束`

关键事实：

- `createNodeVRenderApp({ envParams })` 已覆盖 native canvas smoke。
- legacy-compatible `vglobal.setEnv('node', CanvasPkg)` 后再 `createNodeVRenderApp()` 仍可继承 envParams。
- 推荐本地 / CI 使用与 `canvas` native binding 匹配的 Node 版本；当前验证口径为 Node 20.19.6。

当前结论：

- node 不再是 D3 stable release blocker。
- 后续若 CI 失败，优先检查 Node ABI / `canvas` native binding，而不是回退 node app-scoped contract。

### 3.5 Partially resolved: 多环境支持已分层，Tier 2 仍需真实端 smoke 后升级
...
- app-scoped env installer surface 已覆盖 browser / node / taro / feishu / tt / wx / lynx / harmony
- 根包 public creator 已覆盖 browser / node / taro / feishu / tt / wx / lynx / harmony
- wx / lynx / harmony 已完成真实端 smoke，可进入 stable Tier 1
- taro / feishu / tt 仍保持 Tier 2，等待真实端 smoke 后再升级承诺
...
- 从“是否还能做多环境”角度看，答案是 **能，并已形成 stable support matrix**。
- 从“是否所有历史环境都已一等支持”角度看，答案是 **没有**。
- 当前真正剩余的是 Tier 2 环境真实端 smoke 与 runtime isolation 是否升级为更强承诺。
...
- 当前并非所有摩擦都一样严重。
- stable closeout 后，真正会影响上层“第一次就接对”的 blocker，集中在 **ownership、external stage、Tier 2 端真实 smoke、on-demand 非一等支持、runtime isolation 边界**。
- `app` 概念引入本身、browser/node 双入口本身、root vs core/kits 分层本身，多数仍属于可以接受的迁移成本。

### 4.1 Real blockers

1. **ownership / release contract 仍需要在上层实现中落到 source-level**
   - 归类：`文档问题` + `生命周期/ownership 设计问题`
   - 为什么算 blocker：recreate、页面 teardown、helper 编写、React unmount 都依赖这条 contract。
2. **external stage ownership 当前仍需要跨仓库硬化**
   - 归类：`需要单独专项治理`
   - 为什么算 blocker：这已经不是理论上的“可能踩坑”，而是 coordination 文档里挂账的现存问题。
3. **taro / feishu / tt 仍需真实端 smoke 后才能升级为 Tier 1**
   - 归类：`验证基线问题`
   - 为什么算 blocker：对这些端的上层，当前只能承诺 public creator 与代码路径存在，不能把真实端能力写成 stable 一等承诺。
4. **按需装配能力仍在，但没有对应的一等 app-scoped public surface**
   - 归类：`调用方式约束问题` + `API 表达问题`
   - 为什么算 blocker：对依赖 minimal assembly 的上层，这不是简单改调用名就能完成的迁移。
5. **runtime isolation 边界仍需显式承认**
   - 归类：`运行时 contract 问题`
   - 为什么算 blocker：同一 JS runtime 内混合多 env 的调用方不能默认推断每个 app 都拥有完全隔离 runtime container。
...
- 对 **stable Tier 1 + 默认装配** 的上层，本轮结论仍然是“可接受，可继续迁移”。
- 对 **Tier 2 环境** 的上层，当前 app-scoped 路径还不能被描述成“已经有真实端 stable 一等承诺”。
...
- 它已经覆盖了“推荐入口、compat boundary、root vs core/kits、迁移 grep/checklist、stable support matrix”这些大方向。
- stable closeout 后，文档剩余重点不是 node/type readiness，而是确保上层实现按 matrix 和 ownership contract 落地。

### 5.1 已经足够好的部分

1. **推荐入口清楚**
   - 已经明确新代码优先 stable Tier 1 app creator，见 `D3_UPPER_LAYER_ADOPTION_GUIDE.md`
...
1. **ownership 说明不够**
   - 指南已补 contract；剩余风险是上层 source-level 是否完全按 internal-owned / external-owned 模式落地。
2. **生命周期步骤不够场景化**
   - 指南已补 VChart ownership 矩阵；剩余应由 VChart / React / 端测试项目各自用 smoke 固化。
3. **API 选择 guidance 还不够硬**
   - 普通上层不要混用 root 默认 bootstrap 与 core/kits 手动安装，这一点仍需在上层迁移 review 中强制检查。
4. **Tier 2 路由规则需要继续执行**
   - taro / feishu / tt 在真实端 smoke 前仍不能升级为 stable Tier 1。
5. **on-demand guidance 仍是后续治理**
   - root app creator 是默认全量装配入口，不是细粒度按需装配接口。
6. **multi-env isolation caveat 仍需保留**
   - app-scoped 当前更接近 registry/lifecycle scope，不应默认推断成完全隔离的 per-app/per-env runtime container。
...
3. **`D3_ALPHA_COORDINATION.md`**
   - 当前最有价值的是它保留了 external stage ownership 仍需跨仓库对齐的 caveat。
   - node blocker 已由 stable closeout 关闭，不再按 alpha coordination 的旧状态执行。

本轮判断：

- 现有文档问题已经从“缺少几条硬规则”转为“需要按 stable support matrix 执行并删除/标注历史口径”。
- 因此问题重点不在“再加更多背景”，而在“确保上层迁移时不再引用 browser-alpha 阶段的旧 blocker 列表”。
...
- 当前最优先动作不是重开 API 大设计，而是按 stable support matrix 继续收敛上层接入。
- 第一优先级应是：**VChart app-provider-first / external-stage ownership source-level 落地**，以及 **Tier 2 端 smoke 后的矩阵升级判断**。
- 对按需装配，还需要继续把“能力仍在，但 root creator 不是细粒度按需装配入口”写成硬规则。
- node runtime 与 root creator typing 已不再作为本 review 的待办项。

### 6.1 文档与上层 review 继续执行

1. 保持 `D3_UPPER_LAYER_ADOPTION_GUIDE.md` 中的 ownership contract 和 VChart 场景矩阵为上层 review 标准。
2. 在 compat 小节保持一句更硬的说明：
   - root `createStage()` 仍会工作，但它内部走的是 compat singleton app，不应继续扩进新调用方
3. 保持 multi-env / on-demand routing table：
   - stable Tier 1 默认接入：根包 app creator
   - Tier 2 环境：真实端 smoke 前不能写成一等承诺
   - 强依赖按需装配：当前不要把 root app creator 当成细粒度装配接口
4. 保持 isolation caveat：
...
5. 如果上层明确需要 Tier 2 环境或细粒度装配，应先把自己归类为“高级自定义装配 caller”，不要直接套 stable Tier 1 默认指南。

### 6.3 建议 VRender API / 默认行为优化

1. 保持 compat `createStage()`，但把 deprecation 语义再做强一点
   - 至少要确保上层在类型提示和文档中不会把它误读成仍推荐的新主入口
2. 若后续评估仍显示 single-stage ownership 频繁被误用，再考虑补一个轻量 helper
   - 但这应放在 VChart / React / 多端 smoke 明确复现之后，而不是现在先重做 API
3. 如果 taro / feishu / tt 要升级 stable Tier 1，应补齐真实端 smoke 证据并回填 support matrix
4. 如果按需装配仍是正式产品能力，应补齐细粒度 app-scoped installer surface，或者明确宣告 root app creator 故意只做 full-default path
5. 如果“同进程混合多环境”仍是正式承诺，应评估是否继续接受 `runtimeInstallerContext` / `runtimeGlobal` / `application.*` 的共享单例实现；若接受，也需要把边界文档化

### 6.4 建议单独开任务，不适合当前 alpha 线顺手改

1. **Cross-repo external stage ownership 治理**
   - 目标：把“谁创建，谁释放”写成跨仓库一致 contract，并修正上层现存误释放点
2. **Tier 2 env smoke and upgrade**
   - 目标：决定 taro / feishu / tt 是否进入一等 app-scoped 契约；若要，先补齐真实端 smoke
3. **App-scoped on-demand assembly 治理**
   - 目标：决定“按需装配”是否仍作为正式承诺保留；若保留，就补齐 public installer surface，而不是只剩 legacy/内部 escape hatch
4. **Runtime isolation contract 治理**
   - 目标：决定同一 JS runtime 内多 env 是否需要强隔离；若不承诺，应在用户文档中持续保持 caveat
...
- 当前新的 app-scoped 接入方式，对 **stable Tier 1 主路径** 来说 **总体可接受**，可以继续作为上层仓库的新默认接入方向。
- 但它 **还没有好到“上层 agent 不问人也能稳定一次做对所有场景”**。当前最大的摩擦点不是 app 概念本身，而是：
  - **ownership / lifecycle contract 表达不够清楚**
  - **Tier 2 环境仍需要真实端 smoke 后再升级**
  - **按需装配能力仍在，但不再是新主路径的一等能力**
  - **external stage ownership 与 runtime isolation 仍有真实边界项**

因此当前最合理的优先级是：

1. 推进 VChart app-provider-first / external-stage ownership source-level 对齐
2. 继续保持 stable Tier 1 / Tier 2 / Tier 3 support matrix
3. 对 taro / feishu / tt 真实端 smoke、on-demand app installer、runtime isolation 单独开后续治理任务

如果只问一句“当前新接入方式对上层是否总体可接受”，答案是：

- **在 stable Tier 1 root-package 主路径下，可以接受**
- **在 external-stage、Tier 2 env、on-demand assembly，以及需要同进程混合多环境隔离的场景下，还不能把它描述成已经完全低摩擦**
