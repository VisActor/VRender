...
| BS-P1-009 | path/svg/xml parser 低频化 | core common/path/svg/xml | `Graphic` base 和 root common 可能让 XML/SVG/path parser 进入基础内容路径 | `core/src/graphic/graphic.ts`、`common/xml/*`、`path/*`、`svg/*` | core graphic/common -> parser | 可能影响内部契约，需要谨慎 | Medium | Medium-High | P1 | core tests；path/svg/xml tests；size ledger | VChart 可选 smoke | partial: XML parser 已从 `graphic.ts` / `tools.ts` 静态闭包移出；root barrel 仍公开导出 |
