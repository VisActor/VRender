...
- 在图表 / 表格常规场景中有可说明的收益预估、上层接入方式和用户选择语义。
- 不需要大改架构。
...
- VChart line/simple stats 只作为外部 smoke、回归参照和场景收益样本。
- 新增 optional register / installer / public subpath 前，先看 [scenario value assessment](./VRENDER_SCENARIO_SIZE_VALUE_ASSESSMENT.md)。
...
| BS-P0-006 | 建立图表 / 表格常规场景收益 gate | completed 2026-06-03 | 已记录 VRender source ledger、VChart line/pie/simple/full 场景样本、VTable stats 缺口和后续任务预估收益 |
...
| ID | 任务 | 状态 | 进度记录 |
| --- | --- | --- | --- |
| BS-P0-002 | core graphic/common heavy path 内容瘦身审计 | in-progress | 已完成四片 dead-source 清理：累计减少 core source 49,843 raw / 11,059 gzip；latest 将 XML parser 从 base graphic/tools 静态闭包移到低频 runtime 分支，基础闭包避免 14,134 raw / 5,648 gzip XML parser 内容 |
| BS-P1-009 | path/svg/xml parser 低频化 | deferred | XML parser 已低频化；`path-svg` 经 morph / CustomPath2D reachability 复核后暂停拆分，直接收益 1.7KB gzip 级但风险跨 path/symbol/morph/MotionPath/streamLight |
...
| BS-P0-002 | core graphic/common heavy path 内容瘦身审计 | `@visactor/vrender-core` graphic/common | core 是最大内容 owner；Graphic 基类、path/svg/xml/text/richtext/3D 等可能包含可移出基础路径的代码 | `core/src/graphic/graphic.ts`、`common/xml/*`、`path/*`、`svg/*`、`text/*`、`graphic/graphic-service/graphic-service.ts` | core graphic/common 真实代码内容 | 不应改变 root/default 行为；只删除死码或隔离 optional | High | Medium | P0 | core unit/compile；VRender size ledger before/after | 不需要，VChart 只做 smoke | in-progress: dead-source slices 2026-06-01; segment curve stubs removed; unused graphic-service helper removed; XML parser removed from base static closures 2026-06-02 |
...
| BS-P1-009 | path/svg/xml parser 低频化 | core common/path/svg/xml | `Graphic` base 和 root common 可能让 XML/SVG/path parser 进入基础内容路径 | `core/src/graphic/graphic.ts`、`common/xml/*`、`common/path-svg.ts`、`common/custom-path2d.ts`、`common/morphing-utils.ts`、`animate/custom/morphing.ts`、`animate/custom/motionPath.ts`、`animate/custom/streamLight.ts` | core graphic/common -> parser | 可能影响内部契约，需要谨慎 | Low-Medium for path-svg alone | Medium-High | P1 | core tests；path/svg/xml/morph tests；size ledger | VChart 可选 smoke | deferred: XML parser 已从 `graphic.ts` / `tools.ts` 静态闭包移出；`path-svg` 直接 3,569 raw / 1,735 gzip，且是 `CustomPath2D` / morph / MotionPath / streamLight 基础依赖，暂不拆 |
...
1. 先补 VTable browser / node 常规场景 stats，确认多端 env、components root re-export 和 animate custom 在表格场景中的实际带入。
2. 新增 optional 拆分前必须引用 BS-P0-006 的收益 gate：常规图表 / 表格预估 analyzer gzip 小于 5KB 的任务默认不进 P0。
3. 下一候选优先从 components root-only optional subpath 或 kits env-selected installer 中选一个小 slice；它们在 VChart full-line 和 VTable 源码证据中都有较高 owner 信号。
4. Animate custom 后续只在 VChart/VTable 能按动画效果选择加载时继续，不默认新增更多窄 register。
5. `path-svg`、builtin-symbol、单个小 register 暂缓，除非新 stats 证明场景收益超过 gate。
