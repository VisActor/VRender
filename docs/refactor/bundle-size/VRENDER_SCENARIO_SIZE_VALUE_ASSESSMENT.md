...
| --- | ---: | ---: | --- | --- |
| Components root-only optional subpath | 20-55KB | 以 VChart proxy 判断；table-only 待专门 stats | full-line components 差值 75KB；marker/dataZoom/player/slider/brush 等常规 line 不需要 | VRender public subpath 已补；下一步用 VChart 迁移验证 |
...
1. 先用 VChart line / pie / simple / full 作为主代理，验证 components public subpath 迁移收益，或选择 kits env-selected installer。
