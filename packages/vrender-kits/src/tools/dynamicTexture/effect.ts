...
export function cornerToCenter(
  ...
  const _r = (Math.sin(ratio * 2 * Math.PI - normalizedDistance * 2 * Math.PI) + 1) / 2;
  return _r;
}
...
export function pulseWave(
  ...
  const _r = ((wave + 1) / 2) * (decay * 0.7 + 0.3);
...
export function particleEffect(
  ...
  // 组合所有效果
  const _r = ((movement + 1) / 2) * decay * (0.3 + 0.7 * distanceEffect);
  return Math.max(0, Math.min(1, _r));
}
