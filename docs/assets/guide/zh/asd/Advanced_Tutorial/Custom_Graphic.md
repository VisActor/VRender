...
自定义图元的 renderer、picker 和 contribution 应通过 runtime contribution module 安装。module 接收 VRender 轻量 binding context，注册所需 token，然后在创建 App 前或拿到明确 `app` 后通过 `installRuntimeContributionModule()` 安装。
...
渲染逻辑需要通过 runtime contribution module 注册。如果渲染器继承了需要 contribution 的内置 renderer，需要显式创建依赖。
...
和渲染注册一样，拾取类也通过 runtime contribution module 注册。
...
接下来，我们在代码中加载 `Lottie` 图元相关 runtime contribution，然后使用。这里应使用 `installRuntimeContributionModule`，不要直接把 module load 到旧全局 container。未传 `app` 时，VRender 会把 module 记录给后续 App 创建使用，并刷新已经存在的 shared App；如果 App 已经存在且需要立刻安装到该 App，传入 `{ app, targets }` 即可。
