...
VChart stats 使用本地已有产物，只读解析，未在本轮重新构建。VTable 本轮未发现 ready-made `stats-*.html` 或 `.js.gz` 产物，因此表格场景只给 evidence-backed estimate，后续必须补 VTable stats。
...
VTable 本轮未发现可直接解析的 build stats / gzip 产物。只读源码证据：
...
这说明表格常规场景的高价值方向不是 chart-only 的 axis / legend，而是：
...
表格估算必须等 VTable stats 补齐后再确认；本文件暂不把 VTable 估算写成可验证收益。
...
| --- | ---: | ---: | --- | --- |
| Components root-only optional subpath | 20-55KB | 20-70KB, 待 VTable stats | full-line components 差值 75KB；marker/dataZoom/player/slider/brush 等常规 line 不需要 | 值得继续，但先做 stats-backed 子入口，不改 default |
...
| Kits env / window / canvas profile | 10-30KB | 15-35KB, 待 VTable stats | full-line kits 差值 30KB；VTable 静态导入多端 env | 值得继续，优先 browser/env-selected app installer |
| Animate custom effect profile | 8-25KB | 5-20KB, 待 VTable stats | full-line custom 差值 38KB；line 仍带 morphing/streamLight/tag-points 等 18.6KB | 仅在 VChart/VTable 可按动画效果选择加载时继续 |
...
| Image/resource optional | 3-10KB | 低到中等 | 图表 line image 链路不是最大块；表格 icon/image 常见 | 暂缓，先补 VTable stats |
...
1. 目标场景：line / pie / simple / full / table-browser / table-node 等。
2. 当前 stats：至少一个 VChart 或 VTable 场景的 analyzer gzip，最好同时有 final min gzip。
...
1. 先补 VTable browser 常规场景 stats，确认多端 env 与 components root re-export 的实际影响。
2. 选择一个高价值、低风险 slice：components root-only optional subpath 或 kits env-selected installer。
3. 暂停继续细拆 `path-svg`、builtin-symbol、单个小 register，除非 stats 证明场景收益超过 gate。
