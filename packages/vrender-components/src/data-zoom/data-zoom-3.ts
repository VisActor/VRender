...
export class DataZoom extends AbstractComponent<Required<DataZoomAttributes>> {
  ...
  private _initStates() {
    ...
    this._layoutCache.size = this._isHorizontal ? width : height;
  ...
  private _pointerMove = (e: FederatedPointerEvent) => {
    const { brushSelect, realTime = true } = this.attribute as DataZoomAttributes;
    ...
    if (this._activeState) {
      if (this._activeTag === DataZoomActiveTag.middleHandler) {
        ...
        ({ start, end } = this._renderDragMask(pos));
        shouldRender = false;
      }
    ...
    if (this.state.start !== start || this.state.end !== end) {
      this._setStateAttr(start, end, shouldRender);
      if (realTime) {
  ...
  private _moveZoomWithMiddle(dis: number) {
    ...
    if (dis > 0 && this.state.end + dis > 1) {
      dis = 1 - this.state.end;
    } else if (dis < 0 && this.state.start + dis < 0) {
      dis = -this.state.start;
    }
    return {
      start: clamp(this.state.start + dis, 0, 1),
      end: clamp(this.state.end + dis, 0, 1)
    };
  }
  ...
  /**
   * @description 绘制拖拽时的mask
   */
  private _renderDragMask(pos?: IPointLike) {
    ...
    let end = clamp((currentPos[this._layoutCache.attPos] - position[this._layoutCache.attPos]) / width, 0, 1);
    if (start > end) {
      [start, end] = [end, start];
    }
  ...
  private _onHandlerPointerUp = (e: FederatedPointerEvent) => {
    if (this._activeState) {
      ...
      if (this._activeTag === DataZoomActiveTag.background) {
        this._setStateAttr(this.state.start, this.state.end, true);
      }
    }
   ...
   * 判断文字是否超出datazoom范围
   */
  private _isTextOverflow(componentBoundsLike: IBoundsLike, textBounds: IBoundsLike | null, layout: 'start' | 'end') {
    if (!textBounds) {
      return false;
    }
    if (this._isHorizontal) {
  ...
  private _setTextAttr(startTextBounds: IBoundsLike, endTextBounds: IBoundsLike) {
    ...
    if (this._isHorizontal) {
      ...
      startTextAlignStyle = {
        textAlign: this._isTextOverflow(componentBoundsLike, startTextBounds, 'start') ? 'left' : 'right',
        textBaseline: restStartTextStyle?.textStyle?.textBaseline ?? 'middle'
      };
      endTextAlignStyle = {
        textAlign: this._isTextOverflow(componentBoundsLike, endTextBounds, 'end') ? 'right' : 'left',
      ...
      startTextAlignStyle = {
        ...
        textBaseline: this._isTextOverflow(componentBoundsLike, startTextBounds, 'start') ? 'top' : 'bottom'
      };
      endTextAlignStyle = {
        textAlign: restEndTextStyle?.textStyle?.textAlign ?? 'center',
        textBaseline: this._isTextOverflow(componentBoundsLike, endTextBounds, 'end') ? 'bottom' : 'top'
      };
    }
  ...
  /** state 边界处理 */
  private _setStateAttr(start: number, end: number, shouldRender: boolean) {
  ...
  protected render() {
    ...
    /** drag mask */
    brushSelect && this._renderDragMask();
  ...
  setStartAndEnd(start?: number, end?: number) {
    ...
    if (isValid(start) && isValid(end) && (start !== this.state.start || end !== this.state.end)) {
      ...
      if (startAttr !== this.state.start || endAttr !== this.state.end) {
        this._setStateAttr(start, end, true);
        this._dispatchEvent('change', {
          start,
          end,
          tag: this._activeTag
